...
/**
 * GET /api/bar/stats
 * Get statistics for bar staff dashboard
 */
export async function GET(request: NextRequest) {
  try {
    ...
    // Helper function to check if an item is a bar/drink item
    const isBarItem = (item: any): boolean => {
      ...
      if (foodKeywords.some(keyword => itemName.includes(keyword) || category.includes(keyword))) {
        return false
      }
    ...
    // Calculate department-specific status for each order and filter to only bar orders
    const barOrdersWithStatus = await Promise.all(
      todayOrders.map(async (order) => {
        ...
        // Calculate department-specific status from bar items
    ...
    // Filter out null results (orders without bar items)
    const validBarOrders = barOrdersWithStatus.filter(
    ...
    const pendingCount = validBarOrders.filter(o => o.status === 'pending').length
    const preparingCount = validBarOrders.filter(o => o.status === 'preparing').length
    const readyCount = validBarOrders.filter(o => o.status === 'ready').length
    const deliveredCount = validBarOrders.filter(o => o.status === 'delivered').length
    const completedToday = validBarOrders.filter(o => 
      o.status === 'delivered' || o.status === 'cancelled'
    ).length

    // Calculate average preparation time (for completed orders)
    const completedOrders = validBarOrders.filter(o => o.status === 'delivered' && o.order.delivered_at)
    let avgPrepTime = 0
    ...
    return NextResponse.json({
      stats: {
        ...
        total_today: validBarOrders.length,
        avg_prep_time_minutes: avgPrepTime,
      },
    })
  } catch (error: any) {
    logger.error('Error fetching bar stats:', error)
    return NextResponse.json(
      { error: error.message || 'Failed to fetch bar stats' },
      { status: 500 }
    )
  }
}
