...
export default function BarPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (currentUser.department !== 'bar' && currentUser.department !== 'both') {
        router.push('/')
        return
      }
  ...
  const { data: ordersData, isLoading: ordersLoading } = useQuery<{ orders: BarOrder[], stats: BarStats }>({
    queryKey: ['bar-orders', selectedHotelId],
    queryFn: async () => {
      if (!selectedHotelId) throw new Error('No hotel selected')
      const response = await fetch(`/api/bar/orders?hotel_id=${selectedHotelId}`)
  ...
  // Fetch stats
  const { data: statsData } = useQuery<{ stats: BarStats }>({
    queryKey: ['bar-stats', selectedHotelId],
    queryFn: async () => {
      if (!selectedHotelId) throw new Error('No hotel selected')
      const response = await fetch(`/api/bar/stats?hotel_id=${selectedHotelId}`)
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['bar-orders', selectedHotelId] })
      queryClient.invalidateQueries({ queryKey: ['bar-stats', selectedHotelId] })
    },
  })
  ...
  const hotelName = hotel ? extractTextFromJson(hotel.title) : 'Bar'
    ...
    <div className="min-h-screen w-full bg-gray-900 text-white">
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 py-2">
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <button
            onClick={() => queryClient.invalidateQueries({ queryKey: ['bar-orders', selectedHotelId] })}
        ...
        {ordersLoading && orders.length === 0 ? (
          ...
          <div className="space-y-2">
            {orders.map((order) => {
              ...
              // Check if all bar items are ready (for button state)
              const allBarItemsReady = itemDetails.length > 0 && itemDetails.every((item: any) => 
                item.status === 'ready' || item.status === 'delivered'
              )
              const anyBarItemsPreparing = itemDetails.some((item: any) => item.status === 'preparing')
              const allBarItemsDelivered = itemDetails.length > 0 && itemDetails.every((item: any) => 
              ...
              return (
                ...
                >
                  ...
                  {!allBarItemsDelivered && (
                    <button
                      onClick={() => {
                        // Get item IDs for this order's bar items
                        const itemIds = itemDetails
                          .map((item: any) => item.id)
                          .filter((id: string) => id && id !== '')
                        
                        if (itemIds.length === 0) {
                          console.error('No valid item IDs found for order', order.id)
                          return
                        }
                        ...
                        if (allBarItemsReady && !allBarItemsDelivered) {
                        ...
                        } else if (anyBarItemsPreparing && !allBarItemsReady) {
                      ...
                      className={`w-full py-2 rounded font-semibold text-sm transition-all mt-2 ${
                        allBarItemsDelivered
                          ...
                          : allBarItemsReady && !allBarItemsDelivered
                            ? 'bg-green-600 hover:bg-green-700 text-white active:scale-95'
                            : !anyBarItemsPreparing && !allBarItemsReady
                    ...
                    >
                      ...
                      {!allBarItemsReady && anyBarItemsPreparing && (
                        <span className="flex items-center justify-center gap-1.5">
                          <CheckCircle2 className="w-4 h-4" />
                          Mark Ready
                        </span>
